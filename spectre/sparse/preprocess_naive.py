...
def rolling_min(x: SparseMatrix, k: int, axis: int = 0,
                mode: Optional[str] = None) -> SparseMatrix:
    from spectre.dense.utils import rolling_min
    return _apply(x, axis, lambda a: rolling_min(a, k=k, axis=0, mode=mode))


def rolling_max(x: SparseMatrix, k: int, axis: int = 0,
                mode: Optional[str] = None) -> SparseMatrix:
    from spectre.dense.utils import rolling_max
    return _apply(x, axis, lambda a: rolling_max(a, k=k, axis=0, mode=mode))


def rolling_mean(x: SparseMatrix, k: int, axis: int = 0,
                 mode: Optional[str] = None) -> SparseMatrix:
    from spectre.dense.utils import rolling_mean
    return _apply(x, axis, lambda a: rolling_mean(a, k=k, axis=0, mode=mode))


def rolling_median(x: SparseMatrix, k: int, axis: int = 0,
                   mode: Optional[str] = None) -> SparseMatrix:
    from spectre.dense.utils import rolling_median
    return _apply(x, axis, lambda a: rolling_median(a, k=k, axis=0, mode=mode))


def savgol_filter(x: SparseMatrix, window: int, degree: int,
                  axis: int) -> SparseMatrix:
    from scipy.signal import savgol_filter
    return _apply(x, axis, lambda a: savgol_filter(a, window, degree))
